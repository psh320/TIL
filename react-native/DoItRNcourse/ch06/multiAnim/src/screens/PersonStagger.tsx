...
import { Platform, Image, Text, View, Animated } from 'react-native';
import { Colors } from 'react-native-paper';
import Icon from 'react-native-vector-icons/MaterialCommunityIcons';
import FontawesomeIcon from 'react-native-vector-icons/FontAwesome';
import moment from 'moment-with-locales-es6';
import { useToggle, useLayout, useAnimatedValues } from '../hooks';
...
const PersonStagger: FC<PersonProps> = ({ person, deletePressed }) => {
  ...
  const avatarPressed = useCallback(() => {
    if (Platform.OS === 'ios') {
      Animated.loop(Animated.stagger(delay, [...startAnimations, ...endAnimations])).start(
        toggleStarted,
      );
    } else {
